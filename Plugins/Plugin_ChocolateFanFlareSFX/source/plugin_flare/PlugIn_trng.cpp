...
void cbInitLoadNewLevel(void)
{
	...
	MyData.Save.Global.Counter = 0;
	MyData.Save.Local.SoundIndex = USHRT_MAX;
}
...
int cbFlipEffectMine(WORD FlipIndex, WORD Timer, WORD Extra, WORD ActivationMode)
{
	...
	case FLIPEFFECT_ASSIGN_FIRST:
		MyData.Save.Local.SoundIndex = Timer;
		break;
	case FLIPEFFECT_ASSIGN_SECOND:
		MyData.Save.Local.SoundIndex = Timer + BIT_NINTH;
		break;
...
void InitialiseFlareItem(short ItemIndex)
{
...
{
	Trng.pGlobTomb4->pAdr->pVetSlot[SLOT_FLARE_ITEM].pProcInitialise = InitialiseFlareItem;
}

int cbInitialiseFlareItem(short IndexItem, StrItemTr4 *pItem, WORD CBT_Flags)
{
	pItem->Reserved_34 = MyData.Save.Global.Counter >= 0;
	return SRET_OK;
}

int cbControlFlareItem(short IndexItem, StrItemTr4 *pItem, WORD CBT_Flags)
{
	if (pItem->Reserved_34 && MyData.Save.Local.SoundIndex <= ID_MAXIMUM)
		SoundEffect(MyData.Save.Local.SoundIndex, &pItem->CordX, 0);
	return SRET_OK;
}

int cbControlLara(WORD CBT_Flags, StrItemTr4 *pLara)
{
	if (*Trng.pGlobTomb4->pAdr->pObjInLaraHandsNow == HOLD_FLARE &&
			!PerformConditionTrigger(NULL, CONDITION_SOUND_EFFECT, TIMER_OBJ_GEM_SMASH, 0)) {
		if (*Trng.pGlobTomb4->pAdr->pFlareLifeTime) {
			if (MyData.Save.Local.SoundIndex <= ID_MAXIMUM)
				SoundEffect(MyData.Save.Local.SoundIndex, &pLara->CordX, 0);
			if (MyData.Save.Global.Counter != PERSISTENCE_DEFAULT)
				MyData.Save.Global.Counter = PERSISTENCE_DEFAULT;
		} else {
			if (MyData.Save.Global.Counter >= 0) {
				if (MyData.Save.Global.Counter && MyData.Save.Local.SoundIndex <= ID_MAXIMUM)
					SoundEffect(MyData.Save.Local.SoundIndex, &pLara->CordX, 0);
				--MyData.Save.Global.Counter;
			}
		}
	} else {
		if (MyData.Save.Global.Counter)
			MyData.Save.Global.Counter = 0;
	}
	return SRET_OK;
}
...
// ************  RequireMyCallBacks() function  *****************
	...
	GET_CALLBACK(CB_SLOT_INITIALISE, CBT_FIRST, SLOT_FLARE_ITEM, cbInitialiseFlareItem);
	GET_CALLBACK(CB_SLOT_CONTROL, CBT_FIRST, SLOT_FLARE_ITEM, cbControlFlareItem);
	GET_CALLBACK(CB_LARA_CONTROL, CBT_FIRST, 0, cbControlLara);
