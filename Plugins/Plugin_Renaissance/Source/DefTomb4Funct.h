...
// prototype for CB_DIAGNOSTIC
// received in input:
// WORD DGX_Type  the enumDGX_ type currently selected in the script in DiagnosticType= command
// WORD EDGX_Flags  the enumEDGX_ flags currently enabled in DiagnosticType= command
// char *pCurrentLogText  the text that is going to be displayed on screen (to use only on reading)
// returns:
// char *   pointer to your log text to show on screen, or if you don't want add your log, an empty pointer NULL
typedef char * (__cdecl *CALL_DIAGNOSTIC) (WORD DGX_Type, WORD EDGX_Flags, char *pCurrentLogText);
